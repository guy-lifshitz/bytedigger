...
from bytedigger_engine.package_meta import EXTRA_DBOS, install_hint
...
from bytedigger_engine.lib.phase_sentinel import (  # noqa: F401 — re-export contract (GH612 AC5)
    _CTX_HASH_LEN,
    _PHASE_UUID_SEP,
    _ctx_cfg_sha8,
    execute_engine,
...
def _module_default_db_path() -> Path:
    ...
    zero infra).
    """
    from bytedigger_engine.config_provider import get_config, dbos_db_relpath
...
def _resolve_db_path() -> Path:
    ...
    from bytedigger_engine.config_provider import env_opt
...
def _maybe_evict_on_retry(workflow_uuid: str) -> None:
    ...
    from bytedigger_engine.telemetry_ctx import emit_safe  # CORRECTED §1d cite-verify miss: emit_safe lives in engine_py/telemetry_ctx.py:44
...
def _maybe_emit_durable_resume(workflow_uuid: str) -> None:
    ...
    """
    try:
        from bytedigger_engine.telemetry_ctx import emit_safe
