...
from bytedigger_engine.package_meta import (
...
def check_engine_imports() -> CheckResult:
    try:
        from bytedigger_engine import contracts  # noqa: F401
        from bytedigger_engine import engine
        from bytedigger_engine import event_log  # noqa: F401
        from bytedigger_engine import derive_state  # noqa: F401
        from bytedigger_engine import workflows
...
def check_gates_importable() -> CheckResult:
    # bd#44: these are STRINGS, so no AST-based import rewrite can see them —
    # they are package-qualified by hand and named here so the next move finds
    # them. `__import__("bytedigger_engine.x")` binds the root package, but the
    # check only cares that the import raises nothing.
    modules = [
        "bytedigger_engine.stub_passability",
        "bytedigger_engine.red_lint_checks",
        "bytedigger_engine.verdict_gate",
        "bytedigger_engine.spec_cite",
        "bytedigger_engine.tier_gate",
    ]
...
def check_event_log(dir_path: str) -> CheckResult:
    try:
        from bytedigger_engine import event_log as event_log_mod
