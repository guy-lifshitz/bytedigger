...
from bytedigger_engine.contracts import StepContract, StepResult, WorkflowContext, WorkflowDefinition, format_boundary_error
from bytedigger_engine.config_provider import get_config, timeout_policy_path, repo_top_level_dirs  # noqa: E402  GH285 C2
...
from bytedigger_engine.reject_log import record_plan_review_reject  # noqa: E402  EECA708D
from bytedigger_engine.scope_inverse import scan_scope_inverse  # noqa: E402  EECB919C §1v
from bytedigger_engine.reentry_ac import scan_reentry_ac, stateful_probe  # noqa: E402  GH823 §1ab/§1ac
from bytedigger_engine.helper_extraction import scan_helper_extraction  # noqa: E402  GH863 §1aa
from bytedigger_engine.spec_coverage import scan_spec_coverage, scan_error_taxonomy, scan_finalize_coverage  # noqa: E402  EECB919C §1w / GH824
...
def _parse_cite_unresolved(stdout: str) -> list[dict[str, Any]]:
    ...
    return [
        ...
        if isinstance(f, dict) and f.get("status") in BLOCKING_STATUSES
    ]


def _cite_finding_evidence(finding: dict[str, Any]) -> str:
    """Pure helper (bd#87): one evidence line for a blocking cite finding."""
    status = finding.get("status")
    if status == "no_citations":
        return (
            "no citations to check: the spec references no code file outside "
            "non-python fences"
        )
...
def _emit_cite_status_telemetry(stdout: str, spec_path: Any, rc: int) -> None:
    ...
    payload = {
        ...
        "no_citations": counts.get("no_citations", 0),
...
def _verify_spec_cite_lint(ctx: WorkflowContext, prev: Any) -> StepResult:
    ...
    cite_path = Path(__file__).parent.parent.parent.parent / "spec-cite-lint.py"
...
def _write_review_doc_structured_gate(
    ...
    from bytedigger_engine.lib.plugins.checklist_convergence import extract_structured_findings_raw
...
def _gate_on_review(_ctx: WorkflowContext, prev: Any) -> StepResult:
    ...
    from bytedigger_engine.lib.plugins.checklist_convergence import (
    ...
    if _spec_gate_scratchpad_raw:
        from bytedigger_engine.lib.revise_counter import bump_revise_count  # local import — lib/ on sys.path via engine bootstrap
