"""Central E_* error-code registry for engine_py (GH447).

Pure stdlib, host-decoupled. Enumerates every distinct E_[A-Z0-9_]+ literal
raised across the engine_py production tree, with a one-line docstring
describing the triggering condition. Provides drift detection (--check)
and a reproducible markdown catalogue (--markdown).
"""
    ...
    for dirpath, dirnames, filenames in os.walk(root):
        ...
        for fname in filenames:
            if not fname.endswith(".py"):
                continue
            if fname in ("error_codes.py", "conftest.py"):
                continue
...
def check(root: Path) -> tuple[set[str], set[str]]:
    """Return (unregistered, dead) code sets relative to ERROR_CODES."""
    harvested = harvest_codes(root)
    registered = set(ERROR_CODES.keys())
    unregistered = harvested - registered
    dead = registered - harvested
    return unregistered, dead


def render_markdown() -> str:
    """Render a deterministic markdown catalogue grouped by family prefix."""
    lines = ["# engine_py E_* error-code catalogue", ""]
    families: dict[str, list[str]] = {}
    for code in sorted(ERROR_CODES.keys()):
        parts = code.split("_")
        family = "_".join(parts[:2]) if len(parts) >= 2 else code
        families.setdefault(family, []).append(code)
    for family in sorted(families.keys()):
        lines.append(f"## {family}")
        lines.append("")
        for code in sorted(families[family]):
            lines.append(f"- `{code}` — {ERROR_CODES[code]}")
        lines.append("")
    return "\n".join(lines) + "\n"
...
def main(argv: list[str]) -> int:
    if not argv:
        print("usage: error_codes.py --check [--root PATH] | --markdown", file=sys.stderr)
        return 2
    if argv[0] == "--check":
        root = Path(__file__).resolve().parent
        if len(argv) >= 3 and argv[1] == "--root":
            root = Path(argv[2])
        unregistered, dead = check(root)
        for code in sorted(unregistered):
            print(f"UNREGISTERED {code}")
        for code in sorted(dead):
            print(f"DEAD {code}")
        if unregistered or dead:
            return 1
        print(f"OK {len(ERROR_CODES)} codes")
        return 0
    if argv[0] == "--markdown":
