...
Pre-GREEN predict:
  ...
  AC4: FAIL — `from bytedigger_engine.event_log import HAL_DIR` still present; `resolve_project_root`
       absent.
...
def test_ac2_foreign_git_cwd_not_hal_dir_E8A5ACAB(tmp_path):
    ...
    from bytedigger_engine.lib import project_root as _pr_mod  # noqa: PLC0415
    ...
    with patch.object(_pr_mod, "emit_resolver_resolved", lambda *a, **k: None), \
         patch("bytedigger_engine.workflows.phase_1_discovery.ensure_graph", side_effect=fake_ensure_graph), \
         patch("bytedigger_engine.workflows.phase_1_discovery.invoke_llm_subprocess", mock_invoke):
        phase_1._invoke_discovery_llm(ctx, prev)
    ...
    from bytedigger_engine.event_log import HAL_DIR as _hal_dir  # noqa: PLC0415
...
#   PRESENT: "from bytedigger_engine.lib.project_root import resolve_project_root" AND "resolve_project_root("
#   ABSENT:  "from bytedigger_engine.event_log import HAL_DIR"
#
# Pre-GREEN: FAIL —
#   • "from bytedigger_engine.event_log import HAL_DIR" IS present (L54).
#   • "from bytedigger_engine.lib.project_root import resolve_project_root" is ABSENT (import not yet added).
# ---------------------------------------------------------------------------
...
def test_ac4_import_contract_in_phase1_source_E8A5ACAB():
    ...
    MUST NOT CONTAIN:
      'from bytedigger_engine.event_log import HAL_DIR'

    Pre-GREEN FAIL on absence of resolve_project_root import AND presence of HAL_DIR import.
    """
    from bytedigger_engine.workflows import phase_1_discovery  # noqa: PLC0415
    source_path = Path(phase_1_discovery.__file__)
    source_text = source_path.read_text(encoding="utf-8")

    # Presence gate #1 — required import.
    assert "from bytedigger_engine.lib.project_root import resolve_project_root" in source_text, (
        f"AC4 FAIL: 'from bytedigger_engine.lib.project_root import resolve_project_root' NOT found in "
    ...
    hal_dir_count = source_text.count("from bytedigger_engine.event_log import HAL_DIR")
    assert hal_dir_count == 0, (
        f"AC4 FAIL: 'from bytedigger_engine.event_log import HAL_DIR' found {hal_dir_count} time(s) "
