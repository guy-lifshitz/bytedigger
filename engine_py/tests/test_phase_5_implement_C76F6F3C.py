...
def _semgrep_or_skip() -> None:
    # bd#49: the declaration has to sit in each test's OWN body, and it has to read the
    # frozen host-tool map rather than probe `shutil.which` live — a live probe is exactly
    # what the process-wide `shutil.which` monkeypatches in this file would spoof.
...
def test_verify_red_lint_rules_handles_semgrep_internal_error(tmp_path, monkeypatch):
    ...
    skip_without("semgrep")  # need rules.yml resolution to proceed past skip branches
