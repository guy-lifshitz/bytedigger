...
# bd#49 family: declare the dependency instead of discovering it at assert time.
# `dbos` is an optional Python package, not a host tool, so the frozen host-tool
# map does not cover it; bytedigger's own form is a module-level importorskip
# (precedent: tests/test_gh792_native_sentinel_emit.py).
