...
try:
    ...
    import sibling_test_verifier
    import consumer_dispatch_verifier
    import reachability_verifier
    import helper_extraction_verifier
    import ground_truth_verifier
except ImportError as _e:
    print(f"ERROR: cannot import citation_verifier, sibling_test_verifier, consumer_dispatch_verifier, reachability_verifier, helper_extraction_verifier, ground_truth_verifier from {_LIB_DIR}: {_e}", file=sys.stderr)
    sys.exit(2)
...
def main() -> None:
    ...
    findings = [
        (c.text_offset, f"{basename}:{_rule_id(c.kind, r.evidence)}:{r.evidence}")
        for c, r in pairs
        if not r.verified
    ]

    for f in sibling_test_verifier.find_missing_audits(spec_text, hal_root):
        findings.append((f.offset, f"{basename}:{f.rule_id}:{f.evidence}"))
